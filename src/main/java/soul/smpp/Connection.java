...
import solsmsengine.EventFilter;
import solsmsengine.Sms;
import soul.smpp.event.ReceiverExceptionEvent;
import soul.smpp.event.ReceiverExitEvent;
import soul.smpp.event.ReceiverStartEvent;
import soul.smpp.event.SMPPEvent;
import soul.smpp.message.*;
import soul.smpp.message.tlv.Tag;
import soul.smpp.net.SmscLink;
import soul.smpp.net.TcpLink;
import soul.smpp.util.*;
import soul.smpp.version.SMPPVersion;
import soul.smpp.version.VersionException;
...
//import kg.megacom.ussdmenu.Main;
//import kg.megacom.ussdmenu.SolEventObserver;
...
//    private EventDispatcher eventDispatcher;
     ...
     *            number.
     */
//    public Connection(String host, int port, Main main)
//            throws java.net.UnknownHostException {
//        this(new TcpLink(host, port), false, main);
//    }
     ...
     */
    public Connection(String host, int port, boolean async, Sms ussd, int threadPoolSize)
            throws java.net.UnknownHostException {
        this(new TcpLink(host, port), async, ussd, threadPoolSize);
    }
     ...
     */
//    public Connection(SmscLink link, Main main) {
//        this(link, false, main);
//    }
     ...
     * @param async true for asyncronous communication, false for synchronous.
     */
    public Connection(SmscLink link, boolean async, Sms ussd, int threadPoolSize) {
        this.link = link;
...
//                eventDispatcher = new ThreadedEventDispatcher();
//            }
//        }
//        LOGGER.info("Using event dispatcher " + eventDispatcher.getClass().getName());
//
...
//        eventDispatcher.init();
     ...
     * @throws ie.omk.smpp.version.VersionException if an attempt is made to set the version of the connection
     ...
     *
     * @throws ie.omk.smpp.IllegalStateException if an attempt is made to close the connection while bound to
     *                                           the SMSC.
     ...
     * @throws ie.omk.smpp.UnsupportedOperationException If this connection type does not support operation
     *                                                   <code>r</code>. For example, a receiver link does not
     *                                                   support the submit_sm operation.
     * @throws ie.omk.smpp.AlreadyBoundException         If the request type is a bind packet and this connection is
     *                                                   already bound.
     * @throws ie.omk.smpp.message.SMPPProtocolException If synchronous communications is in use and the incoming
    ...
    public SMPPResponse sendRequest(SMPPRequest request)
        ...
        if (id == SMPPPacket.BIND_RECEIVER || id == SMPPPacket.BIND_TRANSCEIVER || id == SMPPPacket.BIND_TRANSMITTER || id == SMPPPacket.UNBIND) {
     ...
     *
     * @throws ie.omk.smpp.version.VersionException if the version in use does not support the request being
     *                                              sent.
     */
    ...
    protected SMPPResponse waitForResponsePacket(SMPPPacket req)
        ...
        try {
            ...
            while (true) {
                ...
                if (!resp.isRequest() && resp.getSequenceNum() == expectedSeq) {
                    break;
                } else {
     ...
     * @throws java.lang.IllegalArgumentException         if a bad <code>type</code> value is supplied.
     * @throws ie.omk.smpp.VersionException               if an attempt is made to bind as transceiver while using SMPP
     *                                                    version 3.3.
     * @throws ie.omk.smpp.InvalidParameterValueException If any of systemID, password, system type or address range
     ...
     *                                                    output stream.
     * @throws ie.omk.smpp.AlreadyBoundException          If the Connection is already bound.
     * @throws ie.omk.smpp.SMPPProtocolExcpetion          if synchronous communication is in use and the incoming
                ...
                case TRANSCEIVER:
                    if (this.interfaceVersion.isOlder(SMPPVersion.V34)) {
                        throw new VersionException(
                                "Cannot bind as transceiver in " + interfaceVersion.toString());
                    }
     ...
     * @throws ie.omk.smpp.message.SMPPProtocolException If synchronous comms is in use and the incoming unbind_resp
     ...
     *
     * @throws ie.omk.smpp.NotBoundException     if the connection is not currently bound.
     * @throws ie.omk.smpp.AlreadyBoundException if no unbind request has been received from the SMSC.
     * @throws java.io.IOException               If an I/O error occurs while writing the response packet to
     *                                           the network connection.
     */
    public void unbind(UnbindResp ubr) throws java.io.IOException,
            soul.smpp.SMPPException {
        if (state != UNBINDING) {
     ...
     *
     * @throws ie.omk.smpp.AlreadyBoundException if the connection is currently bound to the SMSC.
     */
     ...
     *                                                   connection.
     * @throws ie.omk.smpp.InvalidOperationException     If asynchronous comms is in use.
     * @throws ie.omk.smpp.message.SMPPProtocolException if the incoming data violates the SMPP protocol
     *                                                   specifications.
     */
    ...
    private SMPPPacket readNextPacketInternal()
        ...
        try {
            ...
            if (pak != null) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    StringBuilder b = new StringBuilder("Packet Received: ");
                    int l = pak.getLength();
    ...
    private void processOutboundPacket(SMPPPacket packet) throws IOException {
        ...
        if (!interfaceVersion.isSupported(id)) {
            StringBuffer err = new StringBuffer(120).append(interfaceVersion.toString()).append(" does not support command ID 0x").append(Integer.toHexString(id));
    ...
    private void processInboundBindResp(BindResp resp) {
        ...
        if (n != null) {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("SMSC reports its supported SMPP version as " + smscVersion.toString());
            }

            // Downgrade this connection's version if the SMSC's version is
            // lower.
            if (smscVersion.isOlder(this.interfaceVersion)) {
                LOGGER.info("Downgrading this connection's SMPP version to " + smscVersion.toString());
...
//    }
    ...
    public void run() {
        ...
        try {
            ...
            LOGGER.debug("Didn't find I/O exception config. Using default of " + tooManyIOEx);
        }

//        eventDispatcher.notifyObservers(this, new ReceiverStartEvent(this));
        executors.execute(new EventFilter(ussd, this, new ReceiverStartEvent(this)));
        try {
            while (state != UNBOUND) {
                try {
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Caught a socket timeout exception: " + x.getMessage());
                    }
                    if (state == BINDING) {
                        ...
                        executors.execute(new EventFilter(ussd, this, new ReceiverExceptionEvent(this, x)));
                    }
                    continue;
                } catch (IOException x) {
                    ...
                    ReceiverExceptionEvent ev = new ReceiverExceptionEvent(
                            this, x, state);
                    executors.execute(new EventFilter(ussd, this, ev));
                    smppEx++;
                ...
                executors.execute(new EventFilter(ussd, this, pak));
            } // end while
            if (exitEvent == null) {
        ...
        if (exitEvent != null) {
            executors.execute(new EventFilter(ussd, this, exitEvent));
        }
...
//        eventDispatcher.destroy();
     ...
     * @throws ie.omk.smpp.NotSupportedException if the Connection is currently using an SMPP version which
